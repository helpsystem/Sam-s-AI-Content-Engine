...
export function createOAuthState(uid: string) {
  const payload = Buffer.from(JSON.stringify({ uid, nonce: crypto.randomBytes(16).toString("hex") })).toString("base64url");
...
export function readOAuthState(state: string | null) {
  ...
  return JSON.parse(Buffer.from(payload, "base64url").toString("utf8")) as { uid: string; nonce: string };
}